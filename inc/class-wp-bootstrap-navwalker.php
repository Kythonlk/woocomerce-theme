...
class wp_bootstrap_navwalker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( strcasecmp( $item->attr_title, 'divider' ) == 0 && $depth === 1 ) {
			...
			$classes[] = ' mx-4 menu-item-' . $item->ID;
			...
			$depth_level = apply_filters( 'elector_nav_walker_depth_add_atts', $depth === 0, $depth );

			// If item has_children add atts to a.
			if ( $args->has_children && $depth_level ) {
				$atts['href']   		= $item->url ;
				if ( $args->theme_location == 'all-departments-menu' || $args->theme_location == 'departments-menu' ) {
					$atts['data-bs-toggle']  = 'dropdown-hover';
				} else {
